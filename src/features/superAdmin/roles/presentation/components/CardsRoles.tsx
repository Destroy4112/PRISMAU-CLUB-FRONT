...
import type { CardRol, CardsRolProps } from '../types/menuRol';

function CardsRoles({ cargarRol }: CardsRolProps) {
   ...
   const roles: CardRol[] = [
      ...
      { id: 7, descripcion: 'Administrativo', icono: <FaUser className='text-2xl' />, color: { bg: 'from-green-400 to-emerald-500', icon: 'text-cyan-50' } },
   ];
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-2">
         {roles.map((role) => (
            <div className="relative overflow-hidden rounded-xl transition-all duration-300 cursor-pointer"
               onClick={() => cargarRol(role.id)} key={role.descripcion}>
               <div className={`bg-linear-to-br ${role.color.bg} p-3 flex items-center shadow-lg`}>
                  <div className={`p-2 rounded-full bg-white/30 backdrop-blur-md mr-2 ${role.color.icon}`}>
                     {role.icono}
                  </div>

                  <span className="text-md font-semibold text-white">{role.descripcion}</span>
