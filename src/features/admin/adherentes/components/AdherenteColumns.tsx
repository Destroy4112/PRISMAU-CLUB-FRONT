...
import { FaImages, FaListOl, FaLock, FaUser, FaUserCog, FaUserEdit, FaUserPlus, FaUserSlash, FaUserTimes } from 'react-icons/fa';
import type { ColumnsAsociadoProps } from '../types/adherente';

export default function AdherenteColumns(props: ColumnsAsociadoProps): TableColumn<IAdherente>[] {

    const { cargar, handleDelete, goFamiliares, changeState, changeToAsociado, cargarImagen, reset } = props;
    ...
    return [
        ...
        },
        {
            ...
            cell: row => (
                <div className="flex">
                    ...
                    <button onClick={() => changeToAsociado(row.user_id!)} className='rounded-full w-8 h-8 bg-pink-600 text-white flex justify-center items-center cursor-pointer' title="Cambiar a asociado">
                        <FaUserCog />
                    </button>
                    <button onClick={() => cargarImagen(row.id!, row.imagen!)} className='rounded-full w-8 h-8 bg-purple-600 text-white flex justify-center items-center cursor-pointer' title="Cambiar imagen">
                        <FaImages />
                    </button>
                    <button onClick={() => reset(row.user_id!)} className='rounded-full w-8 h-8 bg-blue-700 text-white flex justify-center items-center cursor-pointer' title="Restablecer clave">
                        <FaLock />
                    </button>
                </div>
            ),
        ...
        },
        {
            ...
            cell: row => (
                <div className="flex justify-center items-center w-full">
                    <span className="bg-purple-100 text-purple-800 text-md font-medium me-2 px-2.5 py-0.5 rounded">
        ...
        {
            name: "Código",
            cell: row => row.Codigo ? row.Codigo : (
