...
import type { ColumnsAdherenteProps } from "../types/adherente";

export default function AdherenteColumns(props: ColumnsAdherenteProps): TableColumn<Adherente>[] {

   const { cargar, handleDelete, goFamiliares, changeState, cargarImagen, reset, changeToAsociado } = props;
      ...
      },
      {
         ...
         cell: (row) => (
            <div className="flex">
               ...
               <button onClick={() => changeToAsociado(row.userId)} className="rounded-full w-8 h-8 bg-red-600 text-white flex justify-center items-center cursor-pointer" title="Cambiar a asociado">
                  <FaUserCog />
               </button>
            </div>
         ),
