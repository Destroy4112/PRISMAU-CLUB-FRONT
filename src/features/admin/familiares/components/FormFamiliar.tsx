...
import { ESTADOS_CIVILES, GENEROS, PARENTESCOS, TIPOS_DOCUMENTOS } from '@models/options/Options.model';
...
function FormFamiliar({ familiar, touched, handleChange }: FormFamiliarProps) {
        ...
        <>
            <div className="max-w-full flex flex-col sm:flex-row sm:space-x-4">
                <InputField label='Nombres' name='Nombre' type='text' icon={FaKeyboard} required
                    value={familiar.Nombre || ""} handleChange={handleChange} placeholder='Escribe los nombres...'
                    id='nombre' classInput={inputValidation(familiar.Nombre, touched)} />
                <InputField label='Apellidos' name='Apellidos' type='text' icon={FaKeyboard} required
                    value={familiar.Apellidos || ""} handleChange={handleChange} placeholder='Escribe los apellidos...'
                    id='apellidos' classInput={inputValidation(familiar.Apellidos, touched)} />
            </div>
            <div className="max-w-full flex flex-col sm:flex-row sm:space-x-4 sm:mt-3">
                <SelectField label='Tipo Documento' name='TipoDocumento' icon={FaIdCard} required id='tipodocumento'
                    items={TIPOS_DOCUMENTOS} value={familiar.TipoDocumento || ""} handleChange={handleChange}
                    clase='sm:w-1/2' classInput={inputValidation(familiar.TipoDocumento, touched)} />
                <InputField label='Número Documento' name='Documento' type='number' icon={FaIdCard} required
                    value={familiar.Documento || ""} handleChange={handleChange}
                    placeholder='Escribe el numero de documento...'
                    id='documento' clase='sm:w-1/2' classInput={inputValidation(familiar.Documento, touched)} />
                <InputField label='Correo' name='Correo' type='email' icon={FaEnvelope} id='correo'
                    value={familiar.Correo || ""} handleChange={handleChange} placeholder='Escribe el correo...' />
            </div>
            <div className="max-w-full flex flex-col sm:flex-row sm:space-x-4 sm:mt-3">
                <InputField label='Teléfono' name='Telefono' type='tel' icon={FaPhoneAlt}
                    value={familiar.Telefono || ""} handleChange={handleChange} id='telefono'
                ...
                <InputField label='Fecha Nacimiento' name='FechaNacimiento' type='date' icon={FaCalendarAlt}
                    value={familiar.FechaNacimiento || ""} handleChange={handleChange}
                ...
                <InputField label='Lugar Nacimiento' name='LugarNacimiento' type='text' icon={FaMapLocation}
                    value={familiar.LugarNacimiento || ""} handleChange={handleChange}
                ...
                <SelectField label='Sexo' name='Sexo' icon={FaMercury} items={GENEROS} required
                    value={familiar.Sexo || ""} handleChange={handleChange} id='sexo'
                    classInput={inputValidation(familiar.Sexo, touched)} />
            </div>
            <div className="max-w-full flex flex-col sm:flex-row sm:space-x-4 sm:mt-3">
                <SelectField label='Estado Civil' name='EstadoCivil' icon={FaUserTag} id='estadocivil'
                    items={ESTADOS_CIVILES} value={familiar.EstadoCivil || ""} handleChange={handleChange} />
                <InputField label='Direccion Residencia' name='Direccion' type='text' icon={FaMapMarkerAlt}
                    value={familiar.DireccionResidencia || ""} handleChange={handleChange}
                    id='direccion' placeholder='Escriba la direccion...' />
                <InputField label='Ciudad Residencia' name='Ciudad' type='text' icon={FaCity}
                    value={familiar.CiudadResidencia || ""} handleChange={handleChange}
                ...
                <SelectField label='Parentesco' name='Parentesco' icon={MdFamilyRestroom} required
                    items={PARENTESCOS} value={familiar.Parentesco || ""} handleChange={handleChange}
                    id='parentesco' classInput={inputValidation(familiar.Parentesco, touched)} />
            </div>
        </>
    );
}
