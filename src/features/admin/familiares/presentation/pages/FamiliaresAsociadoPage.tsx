...
export default function FamiliaresAsociadoPage() {

    const asociado: SocioDetail = useAppLocation().state?.asociado;
    ...
    const { titulo, subtitulo, familiares, isLoading, loading, familiarForm, touched, modals, tituloModal, tituloModalImagen,
        ...
        handleUpdateImagen, closeModalImagen, handleDelete, handleDeleteImagen, handleResetPassword } = useFamiliar(asociado, 'Asociado');

    return (
        <>
            <TituloPage titulo={titulo} subtitulo={subtitulo} icono={<Users className="w-7 h-7" />} color="green" />
            <Contenido>
