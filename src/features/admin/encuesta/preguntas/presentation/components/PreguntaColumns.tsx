...
import { FaComment, FaEdit, FaListOl, FaTrashAlt } from 'react-icons/fa';
import type { Pregunta } from '../../domain/model/pregunta.model';
import type { ColumnsPreguntaProps } from '../types/pregunta';

export default function PreguntaColumns({ cargar, handleDelete, goRespuestas }: ColumnsPreguntaProps): TableColumn<Pregunta>[] {
      ...
      },
      {
         ...
         cell: row => (
            <div className="flex">
               ...
               <button onClick={() => goRespuestas(row)} className='flex items-center justify-center cursor-pointer rounded-full w-8 h-8 bg-yellow-500 text-white' title='Opciones'>
                  <FaComment />
               </button>
            </div>
         ),
